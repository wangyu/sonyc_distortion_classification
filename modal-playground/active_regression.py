...
for idx in range(n_queries):
    query_idx, query_instance = regressor.query(X) #Finds the n_instances most informative point in the data provided by calling the query_strategy function. Returns the queried instances and its indices.
    regressor.teach(X[query_idx].reshape(1, -1), y[query_idx].reshape(1, -1))  # Adds X and y to the known training data and retrains the estimator with the augmented dataset.
